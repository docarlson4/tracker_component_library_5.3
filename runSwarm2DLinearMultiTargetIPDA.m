%%RUNSWARM2DLINEARMULTITARGETIPDA Demonstrate how the Linear Multi-Target
%                       Integrated Probabilistic Data Association Filter
%                       (LM-IPDAF) can be used to track targets performing
...
for curScan = 1:numSamples
    ...
    % tracks over time to draw lines for display. The UUIDs are
    % 36-character strings. For simplicity, we are just using the hash
    % values of the UUIDs so that they can be easily compared with >, = ,<
    % for sorting.
    ...
    % Next, if there are any existing states, we want to predict them to
    % the current time step and update them with the measurements
    if ( curScan > 1 ) && ~isempty( state_st(curScan-1).x )
        ...
        % The gating matrix
        GateMat = false(numTargetsCur, numMeas);
        ...
        for curTar = 1:numTargetsCur
            ...
            [xPredCur,SPredCur] = sqrtDiscKalPred( ...
                x(:,curTar),S(:,:,curTar),F,SQ);
        
            % Update the target existence probabilities with the Markov
            % switching model.
            r(1,curTar) = PStayAlive*r(1,curTar);

            %% Measurement Prediction

            [zPred,PzPred,otherInfo] = sqrtKalmanMeasPred( ...
                xPredCur, SPredCur, H);
            ...
            for curMeas = 1:numMeas
                ...
                %if the target gates.
                innov = zCur(:,curMeas) - zPred;

                Pzz = PzPred + (SRCur(:,:,curMeas)*SRCur(:,:,curMeas)');
